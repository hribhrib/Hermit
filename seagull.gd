...
@onready var _animation_player := $Seagull/AnimationPlayer as AnimationPlayer
@onready var _ray_floor := $Seagull/Armature/RayFloor as RayCast3D
@onready var _ray_wall := $Seagull/Armature/RayWall as RayCast3D
...
func _integrate_forces(state: PhysicsDirectBodyState3D):
	...
	var dir := ($Seagull/Armature as Node3D).get_transform().basis[2].normalized()
	var deaccel_dir := dir
	...
	if advance:
		...
		$Seagull/Armature.set_transform(Transform3D().looking_at(-dir, up))
